...
st.sidebar.markdown(
    "<div style='font-family:JetBrains Mono,monospace;font-size:0.62rem;"
    "text-transform:uppercase;letter-spacing:0.12em;color:#c87941;"
    "margin-bottom:0.4rem'>💧 Water Content (ppm)</div>",
    unsafe_allow_html=True
)
water_filter_on = st.sidebar.checkbox(
    "Enable water content filter",
    value=False,
    help="Filter reports by dissolved water content",
)
include_nd_water = st.sidebar.checkbox(
    "Include ND / not-detected reports",
    value=True,
    help="Show reports where water content was not found in PDF",
)
water_range = st.sidebar.slider(
    ...
    value=(0, 35),
    step=1,
    disabled=not water_filter_on,
    help="IS/IEC limit: ≤35 ppm (220 kV and above), ≤40 ppm (below 220 kV)",
)
...
_ND_SENTINELS = {"ND", "NOT FOUND", "", "NS", "NA", "N/A", "NOT DETECTED", "-", "—", "NF", "NIL", "NONE"}

def resolve_water(data: dict) -> tuple[float | None, str]:
    """
    Triple-check water content from three candidate keys.
    Returns (float_value_or_None, status_string).

    status:
      "ppm"      — clean numeric read, value in ppm
      "nd"       — all reads are ND / sentinel / absent
      "conflict" — reads found but disagree by >5 ppm
      "partial"  — only some reads have values; uses mean of valid ones
    """
    WATER_KEYS = ["water", "water_content", "moisture"]
    ...
    for key in WATER_KEYS:
        ...
        if not raw or raw.upper() in _ND_SENTINELS:
            continue          # this key is ND / absent — skip
        # strip trailing units (ppm, mg/kg, ppm(w/w) …)
        cleaned = raw.upper().replace("PPM", "").replace("MG/KG", "") \
                              .replace("(W/W)", "").replace("W/W", "").strip()
        if not cleaned or cleaned in _ND_SENTINELS:
            continue
        try:
            val = float(cleaned)
            reads.append(val)
            raw_hits.append(f"{key}={raw}")
        except ValueError:
            # non-numeric text in the field — treat as ND
            continue

    if not reads:
        return None, "nd"

    # all three keys (or however many had values) must broadly agree
    if len(reads) > 1 and (max(reads) - min(reads)) > 5.0:
        # disagreement — return the median but flag it
        reads_sorted = sorted(reads)
        median = reads_sorted[len(reads_sorted) // 2]
        return median, "conflict"

    mean_val = sum(reads) / len(reads)
    status = "ppm" if len(reads) == len([k for k in WATER_KEYS if data.get(k)]) else "partial"
...
for i, pdf_file in enumerate(pdfs):
    ...
    if key not in reports:
        ...
        try:
            ...
            water_val, water_status = resolve_water(data)
            reports[key] = dict(
...
if water_filter_on:
    def _water_passes(r: dict) -> bool:
        ...
        if wv is None or ws == "nd":
            return include_nd_water
        return water_range[0] <= wv <= water_range[1]
...
def render_report_card(rep: dict):
    ...
    with st.expander(label, expanded=(auto_expand and sev == "BAD")):
        ...
        # Build water display using triple-checked resolve_water
        _wval, _wstatus = resolve_water(data)
        if _wval is None:
            _water_display = "<span style='color:#484f58'>ND</span>"
        else:
            _wcolor = "#f85149" if _wval > 35 else ("#e3b341" if _wval > 25 else "#3fb950")
            _status_badge = ""
            if _wstatus == "conflict":
                _status_badge = " <span style='color:#e3b341;font-size:0.7rem' title='Reads from multiple keys disagree by >5 ppm'>⚠ conflict</span>"
            elif _wstatus == "partial":
                _status_badge = " <span style='color:#8b949e;font-size:0.7rem' title='Only partial key matches found'>~ partial</span>"
            _water_display = f"<span style='color:{_wcolor};font-weight:700'>{_wval:.1f} ppm</span>{_status_badge}"

        st.markdown(
            f"<div style='display:grid;grid-template-columns:repeat(4,1fr);gap:8px;'>"
            f"<div>{mcell('BDV', v(data,'bdv','kV'))}</div>"
            f"<div><div class='mcell'><div class='mcell-lbl'>Water Content 💧</div>"
