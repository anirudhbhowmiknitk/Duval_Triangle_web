...
Classification (BIS 1866-2017, 40 ppm limit):
  CRITICAL  > 40 ppm
  CAUTION   30-40 ppm
  GOOD      < 30 ppm
...
SEV_ICON  = {"CRITICAL": "🔴", "CAUTION": "🟠", "GOOD": "🟢"}
SEV_ORDER = {"CRITICAL": 0, "CAUTION": 1, "GOOD": 2, "ERR": 3}
SEV_COLOR = {"CRITICAL": "#c0392b", "CAUTION": "#d35400", "GOOD": "#27ae60"}
SEV_BG    = {"CRITICAL": "#fff0f0", "CAUTION": "#fff8f0", "GOOD": "#f0fff4"}
...
def sev_hex(sev):
    return {"CRITICAL":"c0392b","CAUTION":"d35400","GOOD":"27ae60"}.get(sev,"333333")

def icon_text(sev):
    return {"CRITICAL":"[CRITICAL]","CAUTION":"[CAUTION]","GOOD":"[GOOD]"}.get(sev,"")

def _sev_rgb(sev):
    return {
        "CRITICAL": colors.HexColor("#c0392b"),
        "CAUTION":  colors.HexColor("#d35400"),
...
def build_pdf_report(all_reports: list) -> bytes:
    ...
    story.append(Paragraph(
        "Triple-Check Water Monitor  |  BIS 1866-2017 limit: 40 ppm max", small_s))
    story.append(HRFlowable(width="100%", thickness=1.5,
                             color=colors.HexColor("#0d3b6e"), spaceAfter=12))

    # Summary table
    n_c = sum(1 for r in all_reports if r["severity"]=="CRITICAL")
    n_w = sum(1 for r in all_reports if r["severity"]=="CAUTION")
    n_g = sum(1 for r in all_reports if r["severity"]=="GOOD")

    summary_data = [
        ["Total Reports", "Critical (>40 ppm)", "Caution (30-40 ppm)", "Good (<30 ppm)"],
        [str(len(all_reports)), str(n_c), str(n_w), str(n_g)],
    ]
    s_tbl = Table(summary_data, colWidths=[4*cm, 4*cm, 4.5*cm, 4*cm])
    ...
    for rep in all_reports:
        ...
        w_data = [
            ...
            ["FINAL (median)",       w_display,        f"{pct_str} of 40 ppm  |  {agreed_text}"],
        ]
        ...
        final_bg = (colors.HexColor("#f0fff4") if sev=="GOOD" else
                    colors.HexColor("#fff8f0") if sev=="CAUTION" else
        ...
        # Verdict box
        verdict_bg = (colors.HexColor("#fff0f0") if sev=="CRITICAL" else
                      colors.HexColor("#fff8f0") if sev=="CAUTION"  else
                      colors.HexColor("#f0fff4"))
        v_tbl = Table([[Paragraph(
            f"<font color='#{sev_hex(sev)}'><b>{icon_text(sev)} {sev}</b></font>"
            f"  -  Final water: <b>{w_display}</b>  |  Limit: 40 ppm",
            normal_s,
        )]], colWidths=[17*cm])
        ...
        ost_rows_pdf = [
            ...
            ["Water Content",        v(data,"water","ppm"),     "40 ppm max",     _ost_status(data,"water",None,40,"max")],
...
st.set_page_config(
    page_title="Water Content Monitor · Triple-Check",
    page_icon="💧",
    layout="wide",
)
...
<div class="hero-banner">
  ...
  <p>
    ...
    &nbsp;·&nbsp; BIS 1866-2017 limit: <strong>40 ppm</strong>
  </p>
</div>
""", unsafe_allow_html=True)
...
n_c  = sum(1 for r in all_reports if r["severity"]=="CRITICAL")
n_ca = sum(1 for r in all_reports if r["severity"]=="CAUTION")
n_g  = sum(1 for r in all_reports if r["severity"]=="GOOD")
...
<div class="metric-row">
  ...
  <div class="metric-card mc-critical">
    <div class="mc-num">{n_c}</div><div class="mc-lbl">Critical</div></div>
  <div class="metric-card mc-caution">
    <div class="mc-num">{n_ca}</div><div class="mc-lbl">Caution</div></div>
