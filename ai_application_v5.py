...
pdf_files = st.file_uploader(
    "📄 Upload Transformer Oil Report PDFs (one or more)",
    type=["pdf"],
...
if not pdf_files:
    ...
    <div style="border:2px dashed #60C8FF33; border-radius:16px; padding:40px;
      ...
      <div style="font-size:16px; margin-top:10px;">Upload one or more PDF reports to begin analysis</div>
      <div style="font-size:12px; margin-top:6px;">Supports TRU-FIL and SGS/CPRI formats · Multiple files grouped by vendor recommendation</div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
# ──────────────────────────────────────────────────────────────────────────────
# BATCH EXTRACT + ANALYZE
# ──────────────────────────────────────────────────────────────────────────────

# Cache results in session state so we don't re-run on every Streamlit rerun
file_names = [f.name for f in pdf_files]
if "batch_results" not in st.session_state or st.session_state.get("batch_file_names") != file_names:
    batch_results = []  # list of dicts: {filename, data, result, vendor_class, ai_overall}
    progress_bar = st.progress(0, text="Starting batch analysis…")
    for i, pdf_file in enumerate(pdf_files):
        progress_bar.progress((i) / len(pdf_files), text=f"🔍 Extracting {pdf_file.name}…")
        ...
        progress_bar.progress((i + 0.5) / len(pdf_files), text=f"🤖 AI analysis for {pdf_file.name}…")
        try:
            result = groq_analyze(data, api_key)
        except Exception as e:
            st.warning(f"⚠️ Groq API error for {pdf_file.name}: {e}")
            result = {}

        vendor_class = result.get(
    ...
    progress_bar.progress(1.0, text="✅ All files analysed!")
    progress_bar.empty()
    st.session_state["batch_results"] = batch_results
    st.session_state["batch_file_names"] = file_names
else:
    batch_results = st.session_state["batch_results"]

if not batch_results:
    st.error("No reports could be processed.")
    st.stop()

# ──────────────────────────────────────────────────────────────────────────────
# GROUP SUMMARY by VENDOR RECOMMENDATION
# ──────────────────────────────────────────────────────────────────────────────

GROUP_ORDER  = ["BAD", "MILD", "GOOD"]
GROUP_COLORS = {"BAD": "#F44336", "MILD": "#FF9800", "GOOD": "#4CAF50"}
GROUP_ICONS  = {"BAD": "🚨", "MILD": "⚠️", "GOOD": "✅"}
GROUP_BG     = {"BAD": "#3a0a0a", "MILD": "#3a2a00", "GOOD": "#0a2a0a"}

groups: dict[str, list] = {g: [] for g in GROUP_ORDER}
for r in batch_results:
    bucket = r["vendor_class"] if r["vendor_class"] in groups else "MILD"
    groups[bucket].append(r)

st.markdown("## 📊 Fleet Overview — Grouped by Vendor Recommendation")

# Summary bar
col_bad, col_mild, col_good = st.columns(3)
for col, grp in zip([col_bad, col_mild, col_good], GROUP_ORDER):
    cnt = len(groups[grp])
    col.markdown(
        f"""<div style="background:{GROUP_BG[grp]}; border:2px solid {GROUP_COLORS[grp]}55;
                    border-radius:14px; padding:18px; text-align:center;">
          <div style="font-size:28px;">{GROUP_ICONS[grp]}</div>
          <div style="font-size:36px; font-weight:900; color:{GROUP_COLORS[grp]};">{cnt}</div>
          <div style="font-size:13px; color:#aaa; font-weight:bold;">{grp}</div>
          <div style="font-size:11px; color:#666;">transformer{"s" if cnt!=1 else ""}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
# Per-group expandable tables
for grp in GROUP_ORDER:
    items = groups[grp]
    if not items:
        continue
    gc = GROUP_COLORS[grp]
    with st.expander(f"{GROUP_ICONS[grp]} {grp} — {len(items)} report{'s' if len(items)!=1 else ''}", expanded=(grp == "BAD")):
        for item in items:
            d = item["data"]
            r = item["result"]
            eq   = d.get("equipment_designation") or d.get("css_name") or "Unknown"
            loc  = d.get("installation_location") or "—"
            rdat = d.get("report_date") or d.get("sampling_date") or "—"
            sev  = r.get("ai_severity", "—")
            fault = r.get("ai_fault_type", "—")
            ai_ov = item["ai_overall"]
            ai_c  = GROUP_COLORS.get(ai_ov, "#888")
            agree = r.get("agreement", "—")
            ag_c  = {"AGREE":"#4CAF50","PARTIAL":"#FF9800","DISAGREE":"#F44336"}.get(agree,"#888")
            st.markdown(
                f"""<div style="background:#161b22; border-left:4px solid {gc};
                            border-radius:0 10px 10px 0; padding:12px 16px;
                            margin:6px 0; display:flex; flex-wrap:wrap; gap:16px; align-items:center;">
                  <div style="min-width:160px;">
                    <div style="font-size:10px; color:#888;">EQUIPMENT</div>
                    <div style="font-size:15px; font-weight:bold; color:#60C8FF;">{eq}</div>
                    <div style="font-size:11px; color:#666;">{item['filename']}</div>
                  </div>
                  <div style="min-width:120px;">
                    <div style="font-size:10px; color:#888;">LOCATION</div>
                    <div style="font-size:13px; color:#ccc;">{loc}</div>
                  </div>
                  <div style="min-width:100px;">
                    <div style="font-size:10px; color:#888;">DATE</div>
                    <div style="font-size:13px; color:#ccc;">{rdat}</div>
                  </div>
                  <div style="min-width:90px;">
                    <div style="font-size:10px; color:#888;">VENDOR</div>
                    <div style="font-size:14px; font-weight:bold; color:{gc};">{grp}</div>
                  </div>
                  <div style="min-width:90px;">
                    <div style="font-size:10px; color:#888;">AI OVERALL</div>
                    <div style="font-size:14px; font-weight:bold; color:{ai_c};">{ai_ov}</div>
                  </div>
                  <div style="min-width:90px;">
                    <div style="font-size:10px; color:#888;">SEVERITY</div>
                    <div style="font-size:13px; color:#ddd;">{sev}</div>
                  </div>
                  <div style="min-width:140px;">
                    <div style="font-size:10px; color:#888;">FAULT TYPE</div>
                    <div style="font-size:13px; color:#ddd;">{fault}</div>
                  </div>
                  <div style="min-width:90px;">
                    <div style="font-size:10px; color:#888;">AGREEMENT</div>
                    <div style="font-size:13px; font-weight:bold; color:{ag_c};">{agree}</div>
                  </div>
                </div>""",
                unsafe_allow_html=True,
            )
...
# ──────────────────────────────────────────────────────────────────────────────
# PER-REPORT DETAIL — selectbox
# ──────────────────────────────────────────────────────────────────────────────

st.markdown("## 🔎 Individual Report Detail")
report_labels = [
    f"{GROUP_ICONS.get(r['vendor_class'],'?')} [{r['vendor_class']}] {r['data'].get('equipment_designation') or r['data'].get('css_name') or 'Unknown'} — {r['filename']}"
    for r in batch_results
]
selected_idx = st.selectbox("Select a report to inspect:", range(len(batch_results)), format_func=lambda i: report_labels[i])
