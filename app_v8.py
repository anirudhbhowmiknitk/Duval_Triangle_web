...
DANGER_KEYWORDS = [
    ...
    "serious", "severe", "shutdown", "isolate", "emergency",
    "not fit", "unfit", "condemned", "rejected",
    # Oil condition
    "replace oil", "oil replacement", "disastrous", "sludge formation",
    "unfit for retention", "strongly recommended", "immediate corrective",
    "furan analysis", "ir and pi", "drying out",
    # Gas
    "concentration of hydrogen gas is more than",
    "excess of moisture and low bdv",
    "excess of moisture and low ift",
    "higher acidic impurities",
]
MILD_KEYWORDS = [
    # Action
    ...
    "filtration", "filtration is recommended", "oil filtration",
    "establish trend", "trend to reclaim", "reclaim", "reclamation",
    "poor oil", "marginal oil", "excess moisture", "excess of moisture",
    "low bdv", "low ift", "low bdv value", "low ift value",
    "high likelihood of sludge", "sludge", "deterioration",
    "oil replacement may be considered", "if oil replacement is not",
    "comprehensive oil testing", "benchmark values",
    "frequent monitoring", "monitor every", "test the oil every",
    "every six months",
]
NORMAL_KEYWORDS = [
...
# ─── MILD action resources ────────────────────────────────────────────────────
FILTRATION_TIPS = [
    ("🔧", "Hot-Oil Circulation Filtration",
     "Pass oil through a thermovacuum dehydration + Fuller's earth unit at 55–65 °C. "
     "Removes moisture, acidity, and oxidation products. Typically 3–5 passes needed."),
    ("💧", "Vacuum Dehydration / Degassing",
     "Single-stage vacuum dehydration for moisture removal (target <10 ppm H₂O). "
     "Effective when BDV is low but acidity is still within limits."),
    ("🧪", "Fuller's Earth / Activated Alumina Reclamation",
     "Adsorbent reclaiming removes polar degradation products and acids. "
     "Recommended when neutralisation value is 0.15–0.30 mgKOH/g and IFT is marginal."),
    ("📋", "Post-Filtration OST",
     "Submit a fresh sample for full OST + DGA within 48 h of completing filtration "
     "to establish new benchmark values (BDV, IFT, water, acidity, OQI)."),
    ("📊", "Trend Monitoring Schedule",
     "After reclamation: retest at 1 month, 3 months, 6 months. "
     "If OQI stabilises above 100, move to annual routine. "
     "If declining, escalate to oil replacement."),
]
...
/* ═══════════════════════════════════════════════════════
   FILTRATION ACTION PANEL  (MILD reports)
═══════════════════════════════════════════════════════ */
.filt-panel {
...
.filt-card:hover { border-color: #e3b341; }
.filt-card-head {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.72rem;
    font-weight: 600;
...
color:#e6edf3;letter-spacing:0.06em;margin-bottom:0.2rem'>⚙ FILTERS</div>
<div style='font-family:JetBrains Mono,monospace;font-size:0.62rem;color:#6e7681;
letter-spacing:0.1em;text-transform:uppercase;margin-bottom:1rem'>
Report View Controls</div>
""", unsafe_allow_html=True)

# ── Quick-select severity buttons ─────────────────────────────────────────────
st.sidebar.markdown(
    "<div style='font-family:JetBrains Mono,monospace;font-size:0.62rem;"
    "text-transform:uppercase;letter-spacing:0.12em;color:#c87941;"
    "margin-bottom:0.4rem'>Quick Select</div>",
    unsafe_allow_html=True
)
qcol1, qcol2 = st.sidebar.columns(2)
if qcol1.button("🔴 Critical Only"):
    st.session_state["sev_filter"] = ["BAD"]
if qcol2.button("🟠 Monitor Only"):
    st.session_state["sev_filter"] = ["MILD"]
qcol3, qcol4 = st.sidebar.columns(2)
if qcol3.button("🟢 Normal Only"):
    st.session_state["sev_filter"] = ["GOOD"]
if qcol4.button("⬜ Show All"):
    st.session_state["sev_filter"] = ["BAD", "MILD", "GOOD"]

st.sidebar.markdown("<hr style='margin:0.6rem 0'>", unsafe_allow_html=True)

# ── Severity multiselect ───────────────────────────────────────────────────────
severity_filter = st.sidebar.multiselect(
    "🔎 Severity Filter",
    options=["BAD", "MILD", "GOOD"],
    default=st.session_state.get("sev_filter", ["BAD", "MILD", "GOOD"]),
    format_func=lambda x: {"BAD": "🔴 Critical", "MILD": "🟠 Monitor", "GOOD": "🟢 Normal"}[x],
    help="Select one or more severity levels to display",
)
# keep session state in sync with manual multiselect changes
st.session_state["sev_filter"] = severity_filter
...
st.sidebar.markdown(
    "<div style='font-family:JetBrains Mono,monospace;font-size:0.62rem;"
    "text-transform:uppercase;letter-spacing:0.12em;color:#c87941;"
    "margin-bottom:0.6rem'>💧 Water Content Filter</div>",
    unsafe_allow_html=True
)

# Threshold presets — IS 1866 / IEC 60422 limits
_WATER_PRESETS = {
    "All":          (None,  None,  "Show all (no filter)"),
    "≤ 35 ppm":     (None,  35.0,  "220 kV and above (IS 1866)"),
    "≤ 40 ppm":     (None,  40.0,  "Below 220 kV (IS 1866)"),
    "> 35 ppm":     (35.0,  None,  "Exceeds 220 kV limit — flag"),
    "> 40 ppm":     (40.0,  None,  "Exceeds all-class limit — flag"),
    "ND only":      ("nd",  None,  "Reports where water not detected in PDF"),
}

water_preset = st.sidebar.radio(
    "💧 Water Content Filter",
    options=list(_WATER_PRESETS.keys()),
    index=0,
    help="IS 1866: ≤35 ppm (≥220 kV)  |  ≤40 ppm (<220 kV)",
)
_wlo, _whi, _ = _WATER_PRESETS[water_preset]
...
sort_by = st.sidebar.selectbox(
    "↕ Sort Reports By",
    options=["Severity (Critical first)", "Severity (Normal first)", "Filename A→Z", "Duval Zone"],
    index=0,
    help="How to order the displayed reports",
)

st.sidebar.markdown("<hr style='margin:0.6rem 0'>", unsafe_allow_html=True)

# ── Expand options ─────────────────────────────────────────────────────────────
auto_expand = st.sidebar.checkbox(
    "Auto-expand Critical reports",
    value=True,
    help="Critical (BAD) reports open automatically",
)

show_only_critical = False  # legacy compat — handled by severity_filter above
...
_ND_SENTINELS = {"ND", "NOT FOUND", "", "NS", "NA", "N/A", "NOT DETECTED",
                 "-", "—", "NF", "NIL", "NONE", "NOT SPECIFIED"}

# Matches "Water Content By Karl Fischer Method" (case-insensitive, any spacing)
_KF_PATTERN = _re.compile(
    r"water\s+content\s+by\s+karl\s+fischer\s+method"
    r".*?"                          # anything between label and value (units, tabs)
    r"(\d+(?:\.\d+)?)",             # ← the number we want
    _re.IGNORECASE | _re.DOTALL,
)

def _extract_water_from_pdf(pdf_path: str) -> float | None:
    """
    Directly scan the raw PDF text for 'Water Content By Karl Fischer Method'
    and return the number that follows it.
    Tries three strategies across all pages:
      1. pdfplumber extract_text per page
      2. pdfplumber extract_tables (catches tabular layouts)
      3. Concatenated full-doc text
    ...
    try:
        with _pdfplumber.open(pdf_path) as pdf:
            ...
            for page in pdf.pages:
                # ── Strategy 1: plain text per page ───────────────────────────
                txt = page.extract_text() or ""
                full_text_parts.append(txt)
                m = _KF_PATTERN.search(txt)
                if m:
                ...
                for table in page.extract_tables() or []:
                    for row in table:
                        ...
                        m = _KF_PATTERN.search(row_str)
                        if m:
                            candidates.append(float(m.group(1)))

            # ── Strategy 3: full concatenated doc ─────────────────────────────
            full_text = "\n".join(full_text_parts)
            m = _KF_PATTERN.search(full_text)
            if m:
                candidates.append(float(m.group(1)))
    ...
    # All three strategies should agree; if they do, return the value
    unique = list({round(v, 2) for v in candidates})
    if len(unique) == 1:
        return unique[0]
    # Minor float variation — return median
    unique.sort()
    return unique[len(unique) // 2]


def resolve_water(data: dict, pdf_path: str | None = None) -> tuple[float | None, str]:
    """
    Authoritative water content read.

    Step 1 — scan the raw PDF for 'Water Content By Karl Fischer Method' (3 strategies).
    Step 2 — cross-check against data["water"] from the extractor.
    Step 3 — if both agree (within 0.5 ppm): status "ok".
              if only PDF read works:          status "pdf_only".
              if only extractor key works:     status "extractor_only".
              if both present but disagree:    status "mismatch" (trust PDF).
              if neither works:               status "nd".
    """
    # ── PDF direct scan ───────────────────────────────────────────────────────
    pdf_val: float | None = None
    if pdf_path:
        pdf_val = _extract_water_from_pdf(pdf_path)

    # ── Extractor key fallback ────────────────────────────────────────────────
    ext_val: float | None = None
    raw_ext = str(data.get("water", "")).strip()
    if raw_ext and raw_ext.upper() not in _ND_SENTINELS:
        m = _re.search(r"\d+(?:\.\d+)?", raw_ext)
        if m:
            try:
    ...
    # ── Decision ──────────────────────────────────────────────────────────────
    if pdf_val is None and ext_val is None:
        return None, "nd"
    if pdf_val is not None and ext_val is None:
        return pdf_val, "pdf_only"
    if pdf_val is None and ext_val is not None:
        return ext_val, "extractor_only"
    # both present
    if abs(pdf_val - ext_val) <= 0.5:
        return pdf_val, "ok"
    # disagree — trust the direct PDF scan
    return pdf_val, "mismatch"
...
def render_filtration_panel():
    """Render the oil filtration & maintenance action panel for MILD reports."""
    cards = "".join(
        f"<div class='filt-card'>"
        f"<div class='filt-card-head'>{icon} {title}</div>"
        f"<div class='filt-card-body'>{body}</div>"
        f"</div>"
        for icon, title, body in FILTRATION_TIPS
    )
    st.markdown(
        f"<div class='filt-panel'>"
        f"<div class='filt-panel-title'>🛠 Recommended Corrective Actions — Oil Filtration & Monitoring</div>"
        f"<div class='filt-grid'>{cards}</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
for i, pdf_file in enumerate(pdfs):
    ...
    if key not in reports:
        ...
        try:
            ...
            water_val, water_status = resolve_water(data, pdf_path=tmp_path)
            reports[key] = dict(
                ...
                water_val=water_val, water_status=water_status,
            )
...
SORT_KEY = {
    "Severity (Critical first)":  lambda r: SEVERITY_ORDER.get(r["severity"], 99),
    "Severity (Normal first)":    lambda r: -SEVERITY_ORDER.get(r["severity"], 99),
    "Filename A→Z":               lambda r: r["name"].lower(),
    "Duval Zone":                 lambda r: r.get("zone", "ZZ"),
}
all_reports = sorted(
    reports.values(),
    key=SORT_KEY.get(sort_by, lambda r: SEVERITY_ORDER.get(r["severity"], 99))
)
...
if severity_filter:
    filtered_reports = [r for r in filtered_reports if r["severity"] in severity_filter]

if zone_filter:
    filtered_reports = [r for r in filtered_reports if r.get("zone") in zone_filter]

if water_preset != "All":
    def _water_passes(r: dict) -> bool:
        wv = r.get("water_val")
        ws = r.get("water_status", "nd")
        is_nd = (wv is None or ws == "nd")
        if water_preset == "ND only":
            return is_nd
        if is_nd:
            return False   # has no value — exclude when a numeric threshold is active
        if _wlo is not None and wv <= _wlo:
            return False
        if _whi is not None and wv > _whi:
            return False
        return True
    filtered_reports = [r for r in filtered_reports if _water_passes(r)]



# ── No-results message ────────────────────────────────────────────────────────
if all_reports and not filtered_reports:
    st.warning("⚠ No reports match the current filters. Adjust the sidebar filters to see results.")
    st.stop()
...
n_filtered = len(filtered_reports)

filter_note = ""
if n_filtered < n_total:
    filter_note = (
        f"<div style='font-family:JetBrains Mono,monospace;font-size:0.7rem;"
        f"color:#c87941;text-align:center;padding:0.4rem 0 0;letter-spacing:0.05em'>"
        f"⚡ Showing {n_filtered} of {n_total} report{'s' if n_total!=1 else ''} "
        f"(filters active)"
        f"</div>"
    )
...
def render_report_card(rep: dict):
    ...
    with st.expander(label, expanded=(auto_expand and sev == "BAD")):
        ...
        # ── Filtration panel (MILD only) ─────────────────────────────────────
        if sev == "MILD":
            render_filtration_panel()
        ...
        _wval, _wstatus = rep.get("water_val"), rep.get("water_status", "nd")
        if _wval is None:
            _water_display = "<span style='color:#484f58'>ND</span>"
        else:
            _wcolor = "#f85149" if _wval > 40 else ("#e3b341" if _wval > 25 else "#3fb950")
            _badge_map = {
                "ok":              "",
                "pdf_only":        " <span style='color:#8b949e;font-size:0.68rem' title='Read directly from PDF; extractor key absent'>PDF</span>",
                "extractor_only":  " <span style='color:#8b949e;font-size:0.68rem' title='Extractor key only; PDF scan found nothing'>EXT</span>",
                "mismatch":        " <span style='color:#e3b341;font-size:0.68rem' title='PDF scan and extractor disagreed — PDF value used'>⚠</span>",
            }
            _water_display = (
                f"<span style='color:{_wcolor};font-weight:700'>{_wval:.1f} ppm</span>"
                f"{_badge_map.get(_wstatus, '')}"
            )

        st.markdown(
            f"<div style='display:grid;grid-template-columns:repeat(4,1fr);gap:8px;'>"
            f"<div>{mcell('BDV', v(data,'bdv','kV'))}</div>"
            f"<div><div class='mcell'><div class='mcell-lbl'>💧 Water (Karl Fischer)</div>"
            f"<div class='mcell-val'>{_water_display}</div></div></div>"
            f"<div>{mcell('IFT', v(data,'ift','N/m'))}</div>"
            f"<div>{mcell('Neutralization', v(data,'neutralization','mgKOH/g'))}</div>"
            f"</div>",
            unsafe_allow_html=True
        )
        st.markdown(mcell_row([
...
TIERS = [
    ...
    ("MILD", "MONITOR — Filtration / Reclamation / Retest Required", "mild"),
    ("GOOD", "NORMAL — No Action Required",          "good"),
]

for tier, label, css in TIERS:
    group = [r for r in filtered_reports if r["severity"] == tier]
